...
class PrimitiveReplayMoveVerifierTest {
    @Test void actualPrimitiveAndLearnedProgramReplayWithIndependentCostAndForgeryFails() {
        ...
        var move = providers.getFirst().candidates(state, context).moves().getFirst();
        ...
        var fake = SearchMove.from(new Transformation(move.ruleId(), "x+7"), providers.getFirst().descriptor(), 1);
        ...
        for (var candidate : program.candidates(state, context).moves()) {
