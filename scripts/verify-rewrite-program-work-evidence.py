...
def verify_reached_audit(
    ...
    if path_length < 0:
        raise ValueError(f"{case_id}: reached {side} path has no length")
    if correctness == "CONFIRMED":
        if audit_calls != path_length:
            raise ValueError(
                f"{case_id}: confirmed {side} path was not fully audited"
            )
        return
    if correctness == "MISSING_ASSUMPTION":
        if audit_calls != 0:
            raise ValueError(
                f"{case_id}: missing-assumption {side} path was edge-audited"
            )
        return
    if correctness in ("REFUTED", "UNSUPPORTED"):
        if path_length == 0 or not 1 <= audit_calls <= path_length:
            raise ValueError(
                f"{case_id}: {side} fail-closed audit prefix is inconsistent"
            )
        return
    raise ValueError(
        f"{case_id}: reached {side} path has invalid correctness {correctness}"
    )
...
def verify_case(case: dict[str, Any], budget: dict[str, Any]) -> None:
    ...
    if (
        case["baselineExploredStates"]
        != case["baselineOuterSearchWork"]["exploredStates"]
    ):
        raise ValueError(f"{case_id}: baseline explored-state mismatch")
    if (
        case["candidateExploredStates"]
        != case["candidateOuterSearchWork"]["exploredStates"]
    ):
        raise ValueError(f"{case_id}: candidate explored-state mismatch")
    if (
        case["baselineGeneratedTransformations"]
        != case["baselineOuterSearchWork"]["generatedTransformations"]
    ):
        raise ValueError(
            f"{case_id}: baseline generated-transformation mismatch"
        )
    if (
        case["candidateGeneratedTransformations"]
        != case["candidateOuterSearchWork"]["generatedTransformations"]
    ):
        raise ValueError(
            f"{case_id}: candidate generated-transformation mismatch"
        )
    ...
    for side in ("baseline", "candidate"):
        ...
        elif correctness != "NOT_EVALUATED" or audit_calls != 0:
