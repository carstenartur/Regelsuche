#!/usr/bin/env python3
"""Reproduce and independently verify the claim-bounded paper foundation."""
...
def fail(message: str) -> None:
    raise SystemExit(f"paper foundation invalid: {message}")
...
def verify_claim_boundaries(root: Path) -> None:
    ...
    require(
        "Primary tables and figures are intentionally absent" in manuscript,
        "foundation manuscript implies completed primary results",
    )
...
def main() -> int:
    ...
    schema_path = root / "docs/schemas/regelsuche-paper-artifact-manifest-v1.schema.json"
    committed = root / "paper/generated"
    for path in (verifier, manifest_path, schema_path):
        require(
    ...
    require(manifest.get("status") == "FOUNDATION", "paper manifest status is not FOUNDATION")

    with tempfile.TemporaryDirectory(prefix="regelsuche-paper-") as directory:
        temporary_root = Path(directory)
        first = temporary_root / "first"
        second = temporary_root / "second"
        run_verifier(verifier, manifest_path, schema_path, first, True, "first generation")
        run_verifier(verifier, manifest_path, schema_path, second, True, "second generation")
        require_identical(first, second, "clean paper generations")
        require_identical(first, committed, "committed paper foundation")
        require(
            tree_entries(first)
        ...
        verification = load_unique(first / "foundation-verification.json")
        require(
            verification.get("manifestHash") == manifest.get("contentHash"),
            "foundation verification manifest binding drift",
        )
        require(verification.get("status") == "FOUNDATION", "verification status inflated")
        require(
            verification.get("centralClaimsPending") is True,
            "verification no longer marks central claims pending",
        )
        require(
        ...
        require(
            verification.get("requiredEvidenceIssues")
            == sorted(item["issue"] for item in manifest["requiredEvidence"]),
            "verification required-evidence issue binding drift",
        )
    ...
    print("paper-foundation=VERIFIED")
