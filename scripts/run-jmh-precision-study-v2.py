...
from jmh_precision_study_process_v1 import (execute, runner_identity, shared_launch_identity,
                                           source_identity, utc_now)
from jmh_precision_study_v2 import (FROZEN_INPUTS, POLICY_PATH, canonical, cells, evaluate_result,
...
def run_study(root, replicate, output, java=None, prebuilt_jar=None, job_start_epoch=None):
    ...
    manifest = dict(schema="regelsuche.quality.jmh-precision-study-replicate/v2",
    ...
    (output / "manifest.json").write_text(canonical(manifest), encoding="utf-8")
    for key in ("jarError", "integrityError"):
        if key in manifest:
            print(f"replicate={replicate} {key}={manifest[key]}", flush=True)
    for cell in manifest["cells"]:
        if cell["status"] != "MEASURED":
            print(f"replicate={replicate} cell={cell['cellId']} collectionError={cell.get('error')}", flush=True)
    return manifest
