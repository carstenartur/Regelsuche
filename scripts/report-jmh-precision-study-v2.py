...
from jmh_precision_study_v2 import (JAR_SCHEMA, FAMILIES, FROZEN_INPUTS, POLICY_PATH, canonical, cells,
...
def verify_replica(root, directory, policy, inventory):
    ...
    require(manifest.get("schema") == "regelsuche.quality.jmh-precision-study-replicate/v2" and
    ...
    require(jar.get("schema") == JAR_SCHEMA, "unsupported benchmark jar identity schema")
    require(type(jar.get("entryCount")) is int and jar["entryCount"] > 0 and
            type(jar.get("duplicateEntryNameCount")) is int and
            0 <= 2 * jar["duplicateEntryNameCount"] <= jar["entryCount"], "jar entry inventory is incomplete")
...
def analyze(root, directories):
    ...
    content_keys = ("schema", "contentSha256", "entryCount", "duplicateEntryNameCount")
    require(len({tuple(manifest["jar"][key] for key in content_keys) for manifest in manifests}) == 1,
            "benchmark jar content identity differs between replicates")
    ...
    return dict(schema="regelsuche.quality.jmh-precision-study-report/v2", studyId=policy["studyId"],
...
def markdown(report):
    lines = ["# Preregistered JMH precision study v2", "", report.get("claimBoundary", ""), "",
...
def main():
    ...
    try:
        ...
        report = dict(schema="regelsuche.quality.jmh-precision-study-report/v2", collectionStatus="ERROR",
    ...
    print("BEGIN_JMH_PRECISION_STUDY_REPORT_V2")
    print(canonical(report), end="")
    print("END_JMH_PRECISION_STUDY_REPORT_V2")
