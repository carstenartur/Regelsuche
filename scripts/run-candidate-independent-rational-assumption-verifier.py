...
# The historical preregistration predates embedded contentHash fields. Bind its
# reconstructed semantic hash to the already frozen receipt without mutating the
# source file on disk.
_original_validate_frozen_inputs = VERIFIER.validate_frozen_inputs
...
def validate_frozen_inputs(
    ...
    if "contentHash" not in benchmark:
        benchmark["contentHash"] = VERIFIER.semantic_hash(benchmark)
    return _original_validate_frozen_inputs(
        benchmark, corpus, profile, receipt
    )


VERIFIER.validate_frozen_inputs = validate_frozen_inputs

# Discovery reachability requires the target syntax to be constructed. Mere
# algebraic equivalence is still checked, but it is not sufficient to terminate
# the independent search.
def current_constructs_target(
    current: object,
    target: object,
    assumptions: list[str],
) -> bool:
    if VERIFIER.ast_key(current) != VERIFIER.ast_key(target):
        return False
    try:
        VERIFIER.confirm_equivalence_ast(current, target, assumptions)
    except VERIFIER.VerificationError:
        return False
    return True


VERIFIER.current_equivalent_to_target = current_constructs_target

# The same deterministic task evidence is expected to recur in four campaigns.
# The imported verifier historically rejected the second occurrence of any
# semantic root. Keep ordinary set behavior for all other values and perform the
# stronger task-id-to-root consistency check below.
class DeterministicEvidenceSet(set):
...
VERIFIER.set = DeterministicEvidenceSet
...
def validate_run(*args: object, **kwargs: object) -> tuple[int, int]:
    ...
    for campaign in run.get("campaigns", []):
        ...
        for evaluation in campaign.get("taskEvaluations", []):
            ...
            if not isinstance(task_id, str) or not isinstance(root, str):
                VERIFIER.fail("task evaluation identity or root is missing")
            retained = roots_by_task.setdefault(task_id, root)
            if retained != root:
                VERIFIER.fail(
                    f"task evaluation root changed across campaigns for "
                    f"{task_id}: {retained} != {root}"
                )
    ...
    if list(roots_by_task) != VERIFIER.EXPECTED_TASKS:
        VERIFIER.fail(
            f"task evaluation root inventory changed: {list(roots_by_task)}"
        )
    return result
