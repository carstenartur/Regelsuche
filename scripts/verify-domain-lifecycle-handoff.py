...
try:
    ...
    raise SystemExit(
        "jsonschema is required; run scripts/run-domain-lifecycle-handoff-verification.sh"
    ) from error
...
def main() -> int:
    ...
    for name, report in (("expression", expression), ("sequence", sequence)):
        require(
            report.get("sourceKind") == "DOMAIN_DISCOVERY_EVIDENCE",
            f"{name}: source kind drift",
        )
    ...
    for name, report in (("production", production), ("exported", exported)):
        require(
            report.get("sourceKind") == "PRODUCTION_GENERATION_RUN",
            f"{name}: source kind drift",
        )
    ...
    require(
        manifest.get("commitProtocol") == "MANIFEST_LAST_ATOMIC_RENAME",
        "export commit protocol drift",
    )
    ...
    require(
        generation_run.get("contentHash") == manifest.get("generationRunHash"),
        "generation-run hash mismatch",
    )
    require(
        lifecycle_handoff.get("contentHash") == manifest.get("lifecycleHandoffHash"),
        "lifecycle-handoff hash mismatch",
    )
    require(
    ...
    require(
        roles.get("GENERATION_RUN", {}).get("sourceContentHash")
        == manifest.get("generationRunHash"),
        "GENERATION_RUN manifest binding mismatch",
    )
    require(
        roles.get("LIFECYCLE_HANDOFF", {}).get("sourceContentHash")
        == manifest.get("lifecycleHandoffHash"),
    ...
    require(
        sha256(manifest_material) == manifest.get("contentHash"),
        "export manifest content hash drift",
    )
