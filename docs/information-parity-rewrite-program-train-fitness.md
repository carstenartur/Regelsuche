...
exact audit, while every internal program node, source invocation, source
candidate and composed candidate remains charged. The program therefore earns
neither a newly-solved case nor a primitive-step reduction merely because its
outer path is shorter.

A reduction in explored outer states is authorized only when the complete
candidate total, including program formation and audit work, does not exceed the
baseline total. Such a result is a measured strategy benefit under the frozen
ledger, not hidden macro compression. If the complete candidate total is higher,
all resource-reduction components remain zero.
