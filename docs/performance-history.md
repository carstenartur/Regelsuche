...
For average-time measurements, v3 computes `max(0, currentScore - currentScoreError)` and fails only when that decision score is strictly greater than the configured `maximumAllowedScore`; equality passes. This restores the uncertainty-aware decision rule already used by the earlier benchmark contract while keeping the historical v2 verifier and its reports unchanged and reproducible at their original revisions.

A passing regression decision is not automatically a precise performance measurement. When `currentScoreError >= currentScore`, the lower-bound statistic collapses to zero; v3 therefore retains that benchmark as `LOW_PRECISION` in JSON and Markdown evidence. Such a row means that the run did **not demonstrate a material regression under the declared rule**, not that performance is proven unchanged. The rule remains a finite shared-runner ratchet and does not establish cross-hardware absolute performance or statistical significance beyond the recorded JMH uncertainty.
