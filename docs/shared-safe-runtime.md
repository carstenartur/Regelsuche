...
the runtime adapter itself. Historical SAFE_PREPARATION_V2 evidence is unchanged.
The additive [public DIRECT/V4 qualification](safe-runtime-product-qualification.md)
now exercises this actual CLI/Workbench boundary. Its fixed 14-case corpus keeps
SAFE opt-in: it selects 8 of 69 visible rules and spends more logical work while
retaining two additional principal capabilities. Protected FINAL TEST and a
general default qualification remain outside that public contract comparison.
...
Complete runtime exports and CLI/import files are limited to 1,048,556 UTF-8
bytes, reserving 20 bytes for the default 1 MiB HTTP replay envelope. Export
also respects a server's smaller configured request limit. If a valid request
would produce a larger artifact, analysis returns HTTP 413 with
`RUNTIME_ARTIFACT_TOO_LARGE` (CLI exit 1) before exporting any artifact. This is
a transport refusal, not a mathematical no-match or a replay certificate.
Select fewer rules or use a smaller source to obtain complete replayable
evidence. Ordinary HTTP request limits and historical authority algorithms
remain unchanged.
...
from the actual loaded application, authority modules, rule code sources and
active AST visitor contribution code sources, including visitor-only plugins.
Disabled visitor contributions do not add code sources to this execution identity.
