...
The certificate binds the complete pattern-match result, bindings, residual
patterns, diagnostics, rule metadata, assumption lineage, primitive lineage and
work accounting. `verify(...)` checks these identities and then reruns the
complete bounded deterministic plan. Changed paths, assumptions, work,
inventory, principal result or certificate fail closed.

## First SymPy amplification pilot

The first characterization uses the existing independently reimplemented
SymPy/Fu Pythagorean rule from the `sympy-trigonometry` pack:
...
The test explicitly requires the principal-only engine to reject both obscured
positive inputs. The additional successes therefore come from one and two
applications of the general neutral-element preparation, not from a new
trigonometry-specific helper. The different-argument near miss remains
negative.

This is the first amplification characterization, not yet a corpus-wide SymPy
ranking. The follow-up study must group direct and legally obscured examples by
