...
`ExactRationalParseEvidence` has no public constructor. Evidence is issued only
by `ExactRationalDomain` after grammar, bounds and exact-value invariants pass.
A consumer therefore cannot attach an arbitrary certificate hash to a chosen
value and claim that the domain accepted it. Failed parses expose neither a
value nor either hash.
