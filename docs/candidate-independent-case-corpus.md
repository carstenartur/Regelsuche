...
It did not contain the concrete expression tasks, sequence prefixes and holdouts, or replay and paired-utility tasks. Executing campaigns against payloads selected after observing results would turn the benchmark into a post-hoc evaluation.

`case-payload-freeze/v1` repairs that omission **before any evaluated campaign execution**. It does not overwrite or pretend to be part of the original source. The amendment binds the original source semantic hash and Git blob, the previously merged all-incomplete execution foundation and the new concrete corpus.
...
1. validates both Draft 2020-12 schemas and rejects duplicate JSON fields;
2. recomputes every case, corpus and receipt hash;
...
7. verifies sequence observed/holdout separation and ambiguity limits;
8. verifies bounded rational and macro task inventories;
9. recomputes the combined preregistration root;
10. proves that missing cases, cluster substitution, heldout formation leakage and a post-execution freeze receipt fail closed.
...
A green corpus-freeze check establishes only that concrete inputs were fixed while the benchmark still had no evaluated results. It does not establish:
...
The next implementation step may add execution adapters, but it must consume these exact payloads without modifying them after observing outcomes.
