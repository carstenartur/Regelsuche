...
- Die Reduktorstruktur einer gecachten Basis wird ebenfalls vorbereitet und wiederverwendet. Wiederholte Kandidatenprüfungen gegen dasselbe Ideal bezahlen damit nur noch die konkrete Normalformreduktion und gegebenenfalls die diagnostische Interreduktion, nicht erneut die Buchberger-Berechnung und Reduktorindizierung.
- Cache- und Kostenmetriken werden explizit im Ergebnis ausgewiesen: `basisCacheHit`, `basisReuseMode`, `basisCacheSize`, `basisCacheCapacity`, `basisPreparationSteps`, `basisPreparationStepsSaved`, `queryReductionSteps`, `interreductionSteps`, `incrementalBaseGeneratorCount`, `incrementalBaseSize`, `incrementalCandidatePairUpperBound`, `coldInitialPairUpperBound` und die Extension-Generator-Zähler. Dadurch bleibt nachvollziehbar, welcher Rechenaufwand durch exakte oder inkrementelle Wiederverwendung vermieden wurde und wann eine mögliche Wiederverwendung aus Kostengründen verworfen wurde.
- Der wiederverwendbare Vorbereitungsaufwand wird über mehrere inkrementelle Erweiterungen akkumuliert. Ein späterer exakter Cache-Hit kann daher nicht nur die zuletzt ausgeführten Erweiterungsschritte, sondern die gesamte bereits bezahlte Vorbereitung als eingesparte Arbeit ausweisen.
