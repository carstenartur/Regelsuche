...
- Cache- und Kostenmetriken werden explizit im Ergebnis ausgewiesen: `basisCacheHit`, `basisCacheSize`, `basisCacheCapacity`, `basisPreparationSteps`, `basisPreparationStepsSaved`, `queryReductionSteps` und `interreductionSteps`. Dadurch bleibt nachvollziehbar, welcher Rechenaufwand durch Wiederverwendung vermieden wurde.
...
- Der aktuelle Cache erkennt identische kanonisierte Generatorenmengen. Algebraisch äquivalente, aber unterschiedlich erzeugte Ideale sowie echte inkrementelle Ideal-Erweiterungen werden noch nicht zusammengeführt.
