...
class SafeRuntimeAdapterTest {
    ...
    @org.junit.jupiter.params.provider.ValueSource(strings = {"DIRECT_V1", "SAFE_PREPARATION_V3", "SAFE_PREPARATION_V4"})
    void typedSystemKeepsRelationAndCompleteDownstreamReplay(String profile) {
        var adapter = new SafeRuntimeAdapter(List.of(), List.of(), IMPLEMENTATION);
        var request = request(profile, "", List.of("a != 0"));
        ...
        String artifact = adapter.analyze(request);
        assertEquals("SUCCESS", evidence(artifact).get("status"), artifact);
        ...
        if (profile.equals("DIRECT_V1")) {
            assertNull(solving.get("rref"), "recognition-only profile does not run the RREF solver");
        } else {
