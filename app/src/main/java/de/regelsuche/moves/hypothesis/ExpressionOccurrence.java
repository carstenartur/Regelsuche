...
/** One concrete syntax use linked to its shared immutable mathematical value. */
public record ExpressionOccurrence(
    ...
    public ExpressionOccurrence {
        ...
        if (!id.path().equals(position.path())) {
            throw new IllegalArgumentException("occurrence id and tree position must use the same path");
        }
    }
    ...
    public boolean equals(Object other) {
        return this == other || other instanceof ExpressionOccurrence occurrence && id.equals(occurrence.id);
    }
