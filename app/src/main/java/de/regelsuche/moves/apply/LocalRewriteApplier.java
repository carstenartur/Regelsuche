...
public final class LocalRewriteApplier {
     ...
     * {@code rootExpression}.
     *
     * @param rootExpression infix string for the full root expression
     * @param position       subtree position to rewrite
     * @param candidate      candidate move to apply
     * @return rewrite result; check {@link LocalRewriteResult#success()} for outcome
     */
    ...
    /**
     * Applies a group of candidate moves (representing a single logical match)
     * to the subtree at {@code position} within {@code rootExpression}.
     *
     * @param rootExpression infix string for the full root expression
     * @param position       subtree position to rewrite
     * @param candidates     candidate moves belonging to one logical match
     * @return rewrite result; check {@link LocalRewriteResult#success()} for outcome
     */
    ...
    /**
     * Applies a single candidate move to the subtree at {@code position} within
     * an already-parsed {@code root} AST.
     *
     * <p>Use this overload when the caller already holds a parsed {@link Expr} to
     * avoid an unnecessary parse round-trip. The {@link LocalRewriteResult#originalExpression()}
     * field is populated by formatting the provided root.</p>
     *
     * @param root      already-parsed root expression
     * @param position  subtree position to rewrite
     * @param candidate candidate move to apply
     * @return rewrite result; check {@link LocalRewriteResult#success()} for outcome
     */
    public LocalRewriteResult apply(Expr root, TreePosition position, CandidateMove candidate) {
        return apply(root, position, candidate == null ? List.of() : List.of(candidate));
    }

    /**
     * Applies a group of candidate moves (representing a single logical match)
     * to the subtree at {@code position} within an already-parsed {@code root} AST.
     *
     * @param root       already-parsed root expression
     * @param position   subtree position to rewrite
     * @param candidates candidate moves belonging to one logical match
     * @return rewrite result; check {@link LocalRewriteResult#success()} for outcome
     */
    public LocalRewriteResult apply(Expr root, TreePosition position, List<CandidateMove> candidates) {
        List<CandidateMove> source = candidates == null ? List.of() : List.copyOf(candidates);
        if (root == null) {
            return failure(null, position, source, "", "root expression is missing");
        }
        if (position == null) {
            return failure(ExpressionFormatter.format(root), null, source, "", "position is missing");
        }
        if (source.isEmpty()) {
            return failure(ExpressionFormatter.format(root), position, source, "", "candidate move is missing");
        }
    ...
    /**
     * The result of a local rewrite attempt.
     *
     * @param originalExpression the root expression before rewriting (infix string)
     * @param position           the subtree position that was targeted
     * @param subtreeBefore      the subtree text before applying the rule
     * @param subtreeAfter       the subtree text after applying the rule
     *                           ({@code null} when the rewrite failed)
     * @param expressionAfter    the full root expression after the local rewrite
     *                           ({@code null} when the rewrite failed)
     * @param kind               move kind name (e.g. {@code "COMPLETE_SQUARE"})
     * @param bindings           parameter name→value pairs extracted by the enumerator
     * @param success            {@code true} iff the rewrite was applied successfully
     * @param failureReason      human-readable reason string; empty when successful
     */
