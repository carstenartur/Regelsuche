...
public final class SafeRuntimeAdapter implements AutoCloseable {
    ...
    public static SafeRuntimeAdapter open(PluginRuntimeConfig config) {
        ...
        try {
            ...
            // The original direct executor also runs active visitor contributions,
            // including plugins that contribute no rule classes of their own.
            runtime.astVisitorRegistry().registrations().stream()
                .filter(registration -> registration.enabled())
                .forEach(registration -> code.add(registration.visitor().getClass()));
    ...
    public String analyze(SafeRuntimeRequest request) {
        ...
        if (!work.reserveSetup(setup)) {
            ...
            analyzeExpression(request, work, outcomes, authority, principals, preparationRules, support, coverage, sources);
        }
        ...
        return RuntimeArtifactLimits.requireReplayable(canonical(fields("schema", ARTIFACT_SCHEMA,
            "contentHash", hash(canonical(evidence)), "evidence", evidence)) + "\n");
    }

    private void analyzeExpression(SafeRuntimeRequest request, Work work, List<Map<String, Object>> outcomes,
            Map<String, Object> authority, List<RewriteApplicabilitySchema> principals,
            List<RewriteRule> preparationRules, List<RewriteApplicabilitySchema> support,
            List<RewriteApplicabilitySchema.CoverageEntry> coverage, List<DirectSource> sources) {
    ...
    public String replay(Map<String, ?> artifact) {
        RuntimeArtifactLimits.requireReplayable(canonical(artifact) + "\n");
