...
final class DomainExportWorkspaceActions {
    ...
    static void handle(HttpExchange exchange, Path directory, int maxBytes) throws IOException {
        ...
        try {
            ...
            var replayRequest = post && suffix.endsWith("/replay")
                ? readReplayRequest(exchange, maxBytes) : Map.<String, Object>of();
            ...
            if (path.length == 1) sendWorkspace(exchange, 200, workspace);
            else if (path.length == 2 && path[1].equals("replay")) {
                if (!workspace.contentHash().equals(replayRequest.get("expectedWorkspaceHash"))
                        || !workspace.evidence().contentHash().equals(replayRequest.get("expectedEvidenceHash"))) {
                    error(exchange, 409, "SOURCE_BINDING_MISMATCH", "replay request belongs to another source view"); return;
                }
                try {
                ...
                byte[] bytes;
                if (path[2].equals(DomainDiscoveryExport.MANIFEST_FILE_NAME)) bytes = workspace.originalManifestBytes();
                else {
                    var role = Arrays.stream(DomainDiscoveryExport.ArtifactRole.values()).filter(value -> value.fileName().equals(path[2])).findFirst();
            ...
            error(exchange, 400, "INVALID_DOMAIN_EXPORT", "invalid JSON replay request");
    ...
    private static Map<String, Object> readReplayRequest(HttpExchange exchange, int maxBytes) throws IOException {
        ...
        if (!request.keySet().equals(Set.of("expectedWorkspaceHash", "expectedEvidenceHash"))
            ...
            throw new IllegalArgumentException("replay requires exact workspace and source evidence identities");
        }
        return request;
    }
