 ...
 * emitted solely because the local solver proposed one. Direct applications of
 * that operator at nested AST positions are also replayed explicitly, so the
 * cheap direct path remains available before preparation at every inspected
 * position.</p>
 */
public final class RationalCommonDenominatorPreparationTransformationEngine
    ...
    public Execution transformWithEvidence(String expression) {
        ...
        for (PositionedNode positioned : positionedNodes(root)) {
            ...
            observations.add(new PreparationObservation(
                ...
                attempt));
            Transformation candidate = candidateForAttempt(
                root,
                formattedRoot,
                positioned,
                attempt,
    ...
    private Transformation candidateForAttempt(
        Expr root,
        ...
        return switch (attempt.status()) {
            case PREPARED -> preparedTransformation(
                root,
                formattedRoot,
                positioned,
                attempt,
                retainedOutputs);
            case DIRECT_MATCH_AVAILABLE -> directTransformation(
                root,
                formattedRoot,
                positioned,
                retainedOutputs);
    ...
    private Transformation directTransformation(
        Expr root,
        String formattedRoot,
        PositionedNode positioned,
        Set<String> retainedOutputs
    ) {
        Transformation principal = replayDirect(positioned.expression());
        if (principal == null) {
            return null;
        }
        Expr replacement = parseRoot(principal.transformedExpression());
        if (replacement == null) {
            return null;
        }
        ...
        return new Transformation(
            ...
            "direct-local-rational:"
                + principal.applicationKey()
                + ":"
                + positionKey(positioned.path()),
            principal.assumptions(),
            principal.packId(),
            principal.license(),
            principal.primitiveRuleIds());
    }
    ...
    private Transformation replayDirect(Expr subtree) {
        String expression = ExpressionFormatter.format(subtree);
        try {
            return principalEngine.transform(expression).stream()
                .filter(transformation ->
                    RationalCommonDenominatorPreparationSolver.PRINCIPAL_RULE_ID
                        .equals(transformation.rule()))
                .sorted(Comparator
                    .comparing(Transformation::applicationKey)
                    .thenComparing(
                        Transformation::transformedExpression))
                .findFirst()
                .orElse(null);
