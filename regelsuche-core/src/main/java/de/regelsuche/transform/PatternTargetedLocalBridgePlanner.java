...
public final class PatternTargetedLocalBridgePlanner {
     ...
     * excluded from the preparation inventory, even if a caller supplied it a
     * second time.
     */
     ...
     * coordinators. The supplied identity must describe the exact frozen
     * preparation engine/inventory used by the caller.
     */
    public PatternTargetedLocalBridgePlanner(
        RewriteRule principalRule,
        ...
        this.principalRuleHash = RuleInventoryFingerprint.ruleContentHash(
            principalRule);
        this.preparationInventoryHash = requireHash(
    ...
    /**
     * Plans a direct or prepared application without receiving a target result.
     */
    public PlanAttempt plan(
        ...
        PatternMatchAnalyzer.Analysis initialAnalysis = analyze(
            source.ast(),
            safeBudget,
            work);
    ...
    /**
     * Recomputes the deterministic bounded search and rejects any changed path,
     * assumption, inventory identity, replay result, work ledger or certificate.
     */
    ...
    private void collectCandidates(
        ...
        for (int index = 0; index < transformations.size(); index++) {
            ...
            Transformation transformation = transformations.get(index);
            ...
            BridgeStep incoming = BridgeStep.from(
                parent.expression(),
                parsed.expression(),
                transformation,
    ...
    public record BridgeStep(
        ...
        public BridgeStep {
            ...
            if (primitiveRuleIds.isEmpty()
                ...
                throw new IllegalArgumentException(
                    "bridge step primitive lineage must not be empty");
            }
        }

        private static BridgeStep from(
            String expressionBefore,
            String expressionAfter,
            Transformation transformation,
            AssumptionSignature resultingAssumptions
    ...
    public record PreparedBridge(
        ...
        public PreparedBridge {
            if (!PLANNER_ID.equals(plannerId)) {
                throw new IllegalArgumentException(
            ...
            if (preparationSteps.isEmpty()
                    || !terminalAnalysis.matched()
                    || !principalReplay.rule().equals(principalRuleId)
                    || !principalReplay.transformedExpression().equals(
                        resultExpression)
                    || !preparationSteps.getFirst().expressionBefore().equals(
                        sourceExpression)
                    || !preparationSteps.getLast().expressionAfter().equals(
                        terminalPreparedExpression)
                    || preparationSteps.stream()
                        .mapToInt(step -> step.primitiveRuleIds().size())
                        .sum() + principalReplay.primitiveStepCount()
                        > budget.maxPrimitiveSteps() + 1) {
                throw new IllegalArgumentException(
                    "prepared bridge path or principal replay is invalid");
            }
        }
    }
    ...
    public record PlanAttempt(
        ...
        public PlanAttempt {
            ...
            if ((status == Status.UNSUPPORTED) != initialAnalysis.isEmpty()
                    || (status == Status.DIRECT_MATCH_AVAILABLE)
